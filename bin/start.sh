...
then
	...
	nohup java $JAVA_MEM_OPTS $JAVA_OPTS -Ddubbo.shutdown.hook=true -Dapp.workerId=${jvm} -Dbizjar=${bizjar} -cp .:conf:${module}/${jvm}:$CLASSPATH com.alibaba.dubbo.container.Main &
  	tail -f nohup.out
fi
